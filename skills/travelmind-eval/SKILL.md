...
description: |
  TravelMind Agent 质量评测工作流：执行 80 queries × 28 约束评测并与历史基线对比。
...
80 条 × 28 约束standard 查询（覆盖 weather_fit 主战场），约 10-20 分钟。
...
80 queries × 28 约束，约 25-45 分钟（DeepSeek 偶发超时重试属正常）。
...
## 判定参考（Phase 12.28 基线 — 80 queries × 28 约束）

- **Micro** ≥ 83.7% / **Macro** ≥ 61.3%（49/80）为不回退
- LLM 非确定性：单 query 翻转 ±2-3 条属正常波动，chat/multi-city 分类 ±20pp，必要时重跑取均值
- Phase 12.28 大幅增加 hard queries（h01-h17），当前标准组 51.4% 有合理区分度
- **Phase 12.29** 为纯代码质量加固，评测指标不变
